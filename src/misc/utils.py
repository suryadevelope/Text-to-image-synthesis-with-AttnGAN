...
def build_super_images2(real_imgs, captions, cap_lens, ixtoword,
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            #
            PIL_im = Image.fromarray(np.uint8(img))
            reshaped_array = np.reshape(one_map, (512, 512, 3))
            PIL_att = Image.fromarray(np.uint8(reshaped_array))
            PIL_att = PIL_att.resize(PIL_im.size)
            merged = \
                Image.new('RGBA', (vis_size, vis_size), (0, 0, 0, 0))
            ...
            middle_pad_resized = np.reshape(middle_pad, (786,432))
            
            row.append(np.concatenate([one_map, middle_pad_resized], 1))
            # row.append(np.concatenate([one_map, middle_pad], 1))
            #
            row_merge.append(np.concatenate([merged, middle_pad_resized], 1))
            #
            txt = text_map[i * FONT_MAX:(i + 1) * FONT_MAX,
