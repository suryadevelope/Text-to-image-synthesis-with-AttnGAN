                ...
                for i in range(1):  # 16
                    ...
                    for j in range(batch_size):
                        ...
                        for k in range(len(attention_maps)):
                            if len(fake_imgs) > 1:
                                im = fake_imgs[k].detach().cpu()  # Adjusted indexing
                            else:
                                im = fake_imgs[0].detach().cpu()
                            attn_maps = attention_maps[k]
                            att_sze = attn_maps.size(2)
                            
                            # Assuming fake_imgs, captions, and attention_maps are single samples without a batch dimension
                            # Add batch dimension using unsqueeze(0)
                            im = im.unsqueeze(0)
                            captions = captions.unsqueeze(0)
                            attention_maps_batch = attn_maps.unsqueeze(0)
                            
                            # Assuming cap_lens_np is a list/array containing lengths
                            # Using the first element of cap_lens_np assuming it contains the length for a single sample
                            # img_set, sentences = build_super_images2(
                            #     im,
                            #     captions,
                            #     [cap_lens_np[0]],  # Pass length as a list or array
                            #     self.ixtoword,
